...
CONFIG_PATH = Path("/home/topi/Weather/plugins/plugin-setting.json")
SHELL_PATH = Path("/home/topi/Weather/ImageGeneration/run_inky.sh")
...
def handle_button(event):
    ...
    if label =="C":
        data['_pluginType'] = "Weather"
        with open(CONFIG_PATH,"w",encoding="utf-8") as f:
            json.dump(data,f,indent=4)
            
    if label =="D":
         data['_pluginType'] = "Diashow"
         with open(CONFIG_PATH,"w",encoding="utf-8") as f:
              json.dump(data,f,indent=4)

        # Create the file only if it doesn't already exist
    subprocess.run(["/bin/bash",SHELL_PATH],check=True)
    print("AFTER PROCESS....")
